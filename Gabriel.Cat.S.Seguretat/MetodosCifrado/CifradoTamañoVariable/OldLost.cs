...
namespace Gabriel.Cat.S.Seguretat
{
    ...
    public static class OldLost
    {
        ...
        {//level y orden de posicionesPassword
            byte[] dataEncrypted = new byte[LenghtEncrypted(data, password, level, order)];
        ...
        {//level y orden de posicionesPassword
            ...
            byte[] decrypted = new byte[LenghtDecrypted(data, password, level, order)];
            int pos = 0;
            unsafe
            {
                fixed (byte* ptData = data)
